...
# ==========================================
echo -e "\n${YELLOW}[1/4] システムパッケージのインストールを確認しています...${NC}"
...
# ==========================================
echo -e "\n${YELLOW}[2/4] Python 仮想環境のセットアップを行っています...${NC}"
...
# ==========================================
# 3. 自動起動 (.desktop) ファイルの動的生成と配置
# ==========================================
echo -e "\n${YELLOW}[3/4] 自動起動 (常駐化) 設定を行っています...${NC}"
...
Exec=$PROJECT_DIR/venv/bin/python $PROJECT_DIR/voice_input.py
Terminal=false
Categories=Utility;
...
# 4. セットアップ完了と初回起動
# ==========================================
echo -e "\n${YELLOW}[4/4] セットアップがすべて完了しました！${NC}"
...
case "$choice" in
  y|Y|yes|Yes )
    ...
    nohup ./venv/bin/python voice_input.py >/dev/null 2>&1 &
